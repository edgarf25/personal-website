...
import project_img3 from './cloudcliimg.png'
...
import cloud_img from './cloudwebsiteimg.png'
...
const Project_Data = [
    {
        p_no:"01",
        p_name:"Automated Cloud Instance Benchmarking Tool and Analysis Website",
        p_img: project_img3,
        ...
        p_tech:"C#, Terraform, MongoDB, Node.js, Express.js, AWS, Google Cloud, Azure, bash scripting",
        has_link_only: false,
        p_moreinfo: "Getting into the world of cloud is a facinating and exciting however, it could also be overwhelming. In order to tackle this problem I decided to simplify the comple process of comparing virtual machines(VMs) across different cloud providers.To address this, I initially built a solution using Terraform and Bash, but as the scope of the project grew, I transitioned to C# to overcome the limitations of the original implementation and expand its capabilities. \n\nThe tool I developed automates the provisioning and performance testing of VMs across multiple cloud providers. It begins with a CLI-based setup where users securely input their credentials, which are stored in an encrypted .env file. Ensuring accessibility for a wider range of users, I designed the application to work seamlessly on both Linux and Windows, implementing platform-specific adjustments to achieve this compatibility.\n\nOnce configured, the tool automates the creation of essential infrastructure components, such as security groups and resource groups, before launching the VMs. It then conducts a series of performance tests to evaluate metrics like CPU, memory, and disk performance, with the results stored in a MongoDB database hosted on AWS. This centralized data repository enables easy access and analysis.\n\nTo complement the backend functionality, I developed a web application using Node.js and Express.js, hosted on Azure. The web app provides a user-friendly interface to visualize hundreds of performance test results, presenting normalized data and cost-performance comparisons. It helps users to make informed decisions about which VMs best suit their needs. Additionally, the tool supports custom tests, allowing users to focus on specific instances and have the results automatically visualized or compared on the website.\n\n",
        p_website: "https://cloudmeter-gxgfdjdcc8e3gwfz.eastus-01.azurewebsites.net/",
        p_modal_img: cloud_img
    },
    {
        p_no:"02",
